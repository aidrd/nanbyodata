...
(function ($) {
  ...
  function isWindowNavigatorLanguageJa() {
    return (
      windowNavigatorLanguage === 'ja' ||
      windowNavigatorLanguage.toLowerCase() === 'ja-jp'
    );
  }
  function getCurrentLanguage() {
    return isWindowNavigatorLanguageJa() ? 'ja' : 'en';
  }
  var hintText = 'Type in disease name';
  //  var number_of_hits = 'Number of hits [__NUMBER__]';
  var number_of_hits =
    'Number of hits&nbsp;&nbsp;&nbsp;Designated Intractable Disease (DID):[__DID_NUMBER__]&nbsp;&nbsp;&nbsp;Specific Chronic Pediatric Diseases (SCPD):[__CCSD_NUMBER__]';
  if (isWindowNavigatorLanguageJa()) {
    hintText = '難病名を入力';
    //      number_of_hits = 'ヒット件数 [__NUMBER__]';
    number_of_hits =
      'ヒット件数&nbsp;&nbsp;&nbsp;指定難病(指定):[__DID_NUMBER__]&nbsp;&nbsp;&nbsp;小児慢性特定疾病(小慢):[__CCSD_NUMBER__]';
  }
  //  hintText = null;
  ...
  var DEFAULT_SETTINGS = {
    ...
    resultsFormatter: function (item) {
      ...
      var abbr_class =
        classes['dropdownItemWord'] + ' ' + classes['dropdownItemAbbr'];
      if (isWindowNavigatorLanguageJa()) {
        abbr_class = '';
      }
      ...
      var value =
        '<li class="' +
        classes['dropdownItemResult'] +
        '">' +
        '<span class="' +
        classes['dropdownItemWord'] +
        ' ' +
        classes['dropdownItemInformation'] +
        ' glyphicon glyphicon-info-sign"></span>&nbsp;' +
        '<span class="' +
        abbr_class +
        '">' +
        (this.enableHTML ? abbr : _escapeHTML(abbr)) +
        '</span>&nbsp;' +
        '<span class="' +
        classes['dropdownItemWord'] +
  ...
  var DEFAULT_CLASSES = {
    ...
    dropdownItemResultTooltipContentTdColon:
  ...
  $.TokenList = function (input, url_or_data, settings) {
    ...
    this.remove = function (item) {
      ...
      console.log(
        'remove()',
        $(token_list).offset().top,
        $(token_list).outerHeight(),
        $(token_list).innerHeight(),
        $(token_list).height(),
        $(token_list).find('li').height()
      );
    };
    ...
    function resize_input() {
      ...
      if (input_val === (input_val = input_box.val()) && input_val.length) {
        return;
      }

      // Enter new content into resizer and resize input accordingly
      var escaped = (
        settings.placeholder && input_val.length == 0
    ...
    function find_value_and_highlight_term(template, value, term) {
      try {
        ...
        return zenhan(template).replace(
          new RegExp(
            '(?![^&;]+;)(?!<[^<>]*)(' +
              escapeRegExp(escapeHTML(zenhan(value))) +
              ')(?![^<>]*>)(?![^&;]+;)',
            'gi'
          ),
          settings.highlightTerm(value, term)
        );
      } catch (e) {
        console.error(e);
    ...
    function populate_dropdown(query, results) {
      if (results && results.length) {
        ...
        $.each(results, function (index, value) {
          ...
          this_li = find_value_and_highlight_term(
            this_li,
            value['synonym'] instanceof Array
              ? value['synonym'].join(' | ')
              : '',
            query
          );
    ...
    settings.onShowDropdownItem = function (count, did_count, ccsd_count) {
      var node = this;
      var $count_node = $('<div>')
        .addClass(settings.classes.dropdownNumberOfHits)
        .html(
          number_of_hits
            .replace('__NUMBER__', count)
            .replace('__DID_NUMBER__', did_count)
            .replace('__CCSD_NUMBER__', ccsd_count)
        );
    ...
    var showResultsTooltip = function (token_data, node, results) {
      ...
      if (
        ...
        $.isArray(results.selfclass) &&
        results.selfclass.length
      ) {
...
*/
          ...
          //					else{
          if (
            ($.type(selfclass[key]) !== 'string' &&
              $.type(selfclass[key]) !== 'array') ||
            selfclass[key].length == 0
          )
            return;
          value = selfclass[key];
          //					}
          ...
          var label = language[key.toLowerCase()]
            ? language[key.toLowerCase()]
            : key;
          var $contentTr = $('<div>')
            .addClass(settings.classes['dropdownItemResultTooltipContentTr'])
            .addClass(
              settings.classes['dropdownItemResultTooltipContentTr'] +
                '-' +
                key.toLowerCase()
            )
          ...
          $('<div>')
            ...
            .addClass(
              settings.classes['dropdownItemResultTooltipContentTh'] +
                '-' +
                key.toLowerCase()
            )
            .text(label)
          ...
          $('<div>')
            .addClass(
              settings.classes['dropdownItemResultTooltipContentTdColon']
            )
            .text(':')
            .appendTo($contentTr);
          var $value_td = $('<div>')
            ...
            .addClass(
              settings.classes['dropdownItemResultTooltipContentTd'] +
                '-' +
                key.toLowerCase()
            )
            .appendTo($contentTr);
        ...
        $resultsTooltip.css({ visibility: 'hidden' }).show();
        ...
        var content_top =
          $(document).children('html').get(0).scrollTop +
    ...
    $.PopupRelationHPOResultsTooltip = function (node, token_data, options) {
      ...
      if (
        $.isEmptyObject(node) ||
        $.isEmptyObject(token_data) ||
        !$.isPlainObject(token_data)
      )
        return;

      options = options || {};
      if ($.isEmptyObject(options['lang']))
        options['lang'] = isWindowNavigatorLanguageJa() ? 'ja' : 'en';
      if ($.isPlainObject(options)) {
        if ($.type(options['lang']) === 'string') {
          options['hasJA'] =
            options['lang'].toLowerCase() === 'ja' ||
            options['lang'].toLowerCase() === 'jpn'
              ? true
              : false;
          delete options['lang'];
        }
    ...
    $(document).on(
      'mouseover',
      'span.' + settings.classes['dropdownItemInformation'],
      function (e) {
